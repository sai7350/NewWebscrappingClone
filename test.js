...
async function scrapeNoBroker() {
  ...
  try {
    ...
    const bhkaddress = await page.evaluate(() => {
      ...
      return [...document.querySelectorAll("[class=flex]")].map((el) =>
        el.textContent.trim()
      );
    });
// console.log(  " bhk Address Here "+bhkaddress);
const filteredBhkaddress = bhkaddress.filter(line => !line.includes('Post Your Property'));

const splitAndFilteredBhkaddress = filteredBhkaddress.join("Explore Nearby");

// Split the string into an array of lines
const lines = splitAndFilteredBhkaddress.split("\n");

// Remove "Explore Nearby" from each line and trim the lines
const cleanedLines = lines.map(line => line.replace(/Explore Nearby/g, "").trim());

// Join the lines where "Explore Nearby" ends
const mergedString = cleanedLines.join(' ');

// console.log(mergedString);

// Shweta Code Here 
// Define the regular expression pattern to match "sqft...Facing" sentences
const pattern = /\d+\s*sqft[A-Za-z]*Facing/g;

// Split the input text into sentences using the pattern as the delimiter
const sentences = mergedString.split(pattern);

// Remove any leading or trailing whitespace and the pattern from each sentence
const cleanedSentences = sentences.map(sentence => sentence.replace(pattern, '').trim());
...
// const joinedString = splitAndFilteredBhkaddress.join(',');
...
const mergedData = numericPrices.map((price, index) => ({
  price: price,
  address: nonEmptyLines[index],
}));
// console.log( "Merged DAta here"+ JSON.stringify(mergedData));
// End Here
